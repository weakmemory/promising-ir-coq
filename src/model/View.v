...
Module OptTimeMap <: JoinableType.
  ...
  Definition join (lhs rhs:t): t :=
    fun loc => option_join Time.join (lhs loc) (rhs loc).

  Lemma join_comm lhs rhs: join lhs rhs = join rhs lhs.
  Proof.
    apply LocFun.ext. i.
    apply option_join_comm.
    apply Time.join_comm.
  Qed.

  Lemma join_assoc a b c: join (join a b) c = join a (join b c).
  Proof.
    apply LocFun.ext. i.
    apply option_join_assoc.
  ...
  Proof.
    ii. unfold join.
    destruct (lhs loc), (rhs loc); ss; try refl.
    econs. apply Time.join_l.
  Qed.
  ...
  Lemma join_spec lhs rhs o
    ...
    le (join lhs rhs) o.
  Proof.
    ii. eapply option_join_spec; eauto.
    i. eapply Time.join_spec; eauto.
  Qed.
  ...
  Proof.
    ...
    f_equal. apply TimeFacts.antisym; auto.
  Qed.
End OptTimeMap.
