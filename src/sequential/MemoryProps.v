...
Section MEMORYLEMMAS.
  ...
  Lemma cancel_memory_le prom0 mem0 loc from to prom1 mem1
        (MLE: Memory.le prom0 mem0)
        (PROMISE: Memory.cancel prom0 mem0 loc from to prom1 mem1)
    :
    Memory.le prom1 mem1.
  Proof.
    inv PROMISE. ii.
    erewrite (@Memory.remove_o _ mem0); eauto.
    erewrite (@Memory.remove_o _ prom0) in LHS; eauto. des_ifs.
    eapply MLE; eauto.
  Qed.

  Lemma reserve_memory_le prom0 mem0 loc from to prom1 mem1
        (MLE: Memory.le prom0 mem0)
        (PROMISE: Memory.reserve prom0 mem0 loc from to prom1 mem1)
    :
    Memory.le prom1 mem1.
  Proof.
    inv PROMISE. ii.
    erewrite (@Memory.add_o _ mem0); eauto.
    erewrite (@Memory.add_o _ prom0) in LHS; eauto. des_ifs.
    eapply MLE; eauto.
  Qed.
  ...
  Lemma step_promises_le lang (th0 th1: Thread.t lang) e
        (MLE: Memory.le (Local.reserves (Thread.local th0)) (Global.memory (Thread.global th0)))
        (STEP: Thread.step e th0 th1)
  :
    Memory.le (Local.reserves (Thread.local th1)) (Global.memory (Thread.global th1)).
  Proof.
    inv STEP.
    { inv LOCAL.
      { inv LOCAL0; ss. }
      { inv LOCAL0. eapply reserve_memory_le; eauto. }
      { inv LOCAL0. eapply cancel_memory_le; eauto. }
    }
    { inv LOCAL; ss.
      { inv LOCAL0; ss. }
      { inv LOCAL0. etrans; eauto. eapply Memory.add_le; eauto. }
      { inv LOCAL1. inv LOCAL2. ss. etrans; eauto. eapply Memory.add_le; eauto. }
      { inv LOCAL0; ss. }
      { inv LOCAL0; ss. }
    }
  Qed.
  ...
  Lemma memory_add_remove mem0 loc from to msg mem1
        (ADD: Memory.add mem0 loc from to msg mem1)
    :
    Memory.remove mem1 loc from to msg mem0.
  Proof.
    hexploit Memory.remove_exists.
    { eapply Memory.add_get0; eauto. }
    i. des. erewrite (@Memory.ext mem0); eauto.
    i. erewrite (@Memory.remove_o mem2); eauto.
    erewrite (@Memory.add_o mem1); eauto. des_ifs.
    ss. des; clarify. eapply Memory.add_get0; eauto.
  Qed.
...
Section VIEW.
  ...
  Lemma write_tview_mon_same_ord
        tview1 tview2 loc ts ord
        (TVIEW: TView.le tview1 tview2)
    :
      TView.le
        (TView.write_tview tview1 loc ts ord)
        (TView.write_tview tview2 loc ts ord).
  Proof.
...
Section UNCHANGABLES.
  ...
  Lemma unchangable_reserve mem0 prom0 loc from to prom1 mem1
        (PROMISE: Memory.reserve prom0 mem0 loc from to prom1 mem1)
    :
  ...
  Lemma unchangable_cancel mem0 prom0 loc from to prom1 mem1
        (PROMISE: Memory.cancel prom0 mem0 loc from to prom1 mem1)
    :
    unchangable mem0 prom0 <4= unchangable mem1 prom1.
  Proof.
    ...
    + erewrite Memory.remove_o; eauto.
      eapply Memory.remove_get0 in RSV. des_ifs.
  Qed.

  Lemma unchangable_add mem0 prom loc from to msg mem1
        (ADD: Memory.add mem0 loc from to msg mem1)
    :
      unchangable mem0 prom <4= unchangable mem1 prom.
  Proof.
  ...
  Lemma unchangable_internal_step e lc0 gl0 lc1 gl1
        (STEP: Local.internal_step e lc0 gl0 lc1 gl1)
    :
      unchangable gl0.(Global.memory) lc0.(Local.reserves) <4=
      unchangable gl1.(Global.memory) lc1.(Local.reserves).
  Proof.
    inv STEP.
    { inv LOCAL; ss. }
    { inv LOCAL; ss. eapply unchangable_reserve; eauto. }
    { inv LOCAL; ss. eapply unchangable_cancel; eauto. }
  Qed.

  Lemma unchangable_program_step e lc0 gl0 lc1 gl1
  ...
  Proof.
    inv STEP; ss.
    { inv LOCAL; ss. }
    { inv LOCAL; ss. eapply unchangable_add; eauto. }
    { inv LOCAL1; ss. inv LOCAL2; ss. eapply unchangable_add; eauto. }
    { inv LOCAL; ss. }
    { inv LOCAL; ss. }
  Qed.
  ...
  Lemma unchangable_increase e lang (th0 th1: Thread.t lang)
        (STEP: Thread.step e th0 th1)
    :
      unchangable th0.(Thread.global).(Global.memory) (Local.reserves (Thread.local th0)) <4=
      unchangable th1.(Thread.global).(Global.memory) (Local.reserves (Thread.local th1)).
  Proof.
    inv STEP; ss.
    - eapply unchangable_internal_step; eauto.
    - eapply unchangable_program_step; eauto.
  Qed.
  ...
  Proof.
    ...
    inv H. eapply unchangable_increase; eauto.
  Qed.
...
Proof.
  ...
  { eapply cap_covered; eauto. }
  {
    ...
    - etrans; eauto. erewrite <- Memory.cap_max_ts; eauto. eapply CLOSED.
  }
Qed.
...
Section RESERVEFUTURE.
  ...
  | reserve_future_memory_reserve
      prom0 mem0 prom1 mem1 prom2 mem2
      loc from to
      (HD: Memory.reserve prom0 mem0 loc from to prom1 mem1)
      (TL: reserve_future_memory prom1 mem1 prom2 mem2)
    :
    reserve_future_memory
      prom0 mem0 prom2 mem2
  | reserve_future_memory_cancel
      prom0 mem0 prom1 mem1 prom2 mem2
      loc from to
      (HD: Memory.cancel prom0 mem0 loc from to prom1 mem1)
      (TL: reserve_future_memory prom1 mem1 prom2 mem2)
    :
    reserve_future_memory
      prom0 mem0 prom2 mem2
  ...
  Proof.
    ginduction FUTURE; auto.
    { i. transitivity mem1; auto.
      inv HD; clarify. econs; [|refl]. econs 1; eauto.
    }
    { i. transitivity mem1; auto.
      inv HD; clarify. econs; [|refl]. econs 2; eauto.
    }
  Qed.

  Lemma reserve_future_messages_le prom0 mem0 prom1 mem1
        (FUTURE: reserve_future_memory prom0 mem0 prom1 mem1)
    :
      Memory.messages_le mem1 mem0.
  Proof.
    ginduction FUTURE; auto.
    { refl. }
    { etrans; eauto. ii. inv HD.
      erewrite (@Memory.add_o _ mem0) in LHS; eauto. des_ifs.
    }
    { etrans; eauto. ii. inv HD.
      erewrite (@Memory.remove_o _ mem0) in LHS; eauto. des_ifs.
    }
  Qed.
  ...
  Lemma reserve_future_memory_le prom0 mem0 prom1 mem1
    ...
    :
      Memory.le prom1 mem1.
  Proof.
    ginduction FUTURE; eauto.
    { i. inv HD. eapply IHFUTURE. ii.
      erewrite (@Memory.add_o _ prom0) in LHS; eauto.
      erewrite (@Memory.add_o _ mem0); eauto. des_ifs. apply MLE; auto.
    }
    { i. inv HD. eapply IHFUTURE. ii.
      erewrite (@Memory.remove_o _ prom0) in LHS; eauto.
      erewrite (@Memory.remove_o _ mem0); eauto. des_ifs. apply MLE; auto.
    }
  Qed.
  ...
  Proof.
    ...
    { eapply IHFUTURE. inv HD. eapply Memory.add_finite; eauto. }
    { eapply IHFUTURE. inv HD. eapply Memory.remove_finite; eauto. }
  Qed.
  ...
  Proof.
    ginduction FUTURE; eauto.
    { inv HD. i. eapply IHFUTURE. eapply Memory.add_closed; eauto. }
    { inv HD. i. eapply IHFUTURE. eapply Memory.remove_closed; eauto. }
  Qed.
  ...
  | cancel_future_memory_step
      prom0 mem0 prom1 mem1 prom2 mem2
      from to
      (CANCEL: Memory.cancel prom0 mem0 loc from to prom1 mem1)
  ...
  | reserve_add_future_memory_step
      prom0 mem0 prom1 mem1 prom2 mem2
      from to
      (ADD: Memory.reserve prom0 mem0 loc from to prom1 mem1)
  ...
  Proof.
    ginduction FUTURE; auto; i.
    { apply IHFUTURE; auto.
      inv HD; des; clarify.
      erewrite Memory.add_o; eauto.
      des_ifs. ss. des; clarify.
      eapply Memory.add_get0 in RSV. des. clarify.
    }
    { apply IHFUTURE; auto.
      inv HD; des; clarify.
      erewrite Memory.remove_o; eauto.
  ...
  Proof.
    ginduction FUTURE; eauto.
    { i. eapply IHFUTURE. eapply unchangable_reserve; eauto. }
    { i. eapply IHFUTURE. eapply unchangable_cancel; eauto. }
  Qed.

  Lemma reserve_future_memory_future
        vw sc gprm prm rsv0 mem0 rsv1 mem1
        (LOCAL: Local.wf (Local.mk vw prm rsv0) (Global.mk sc gprm mem0))
        (MEM: Global.wf (Global.mk sc gprm mem0))
        (FUTURE: reserve_future_memory rsv0 mem0 rsv1 mem1)
    :
      (<<LOCAL: Local.wf (Local.mk vw prm rsv1) (Global.mk sc gprm mem1)>>) /\
      (<<MEM: Global.wf (Global.mk sc gprm mem1)>>).
  Proof.
    ginduction FUTURE; eauto.
    { i. hexploit Local.internal_step_future; eauto.
      { econs 2; eauto. }
      i. des; auto.
    }
    { i. hexploit Local.internal_step_future; eauto.
...
Section SEMICLOSED.
  ...
  Definition semi_closed_timemap
    ...
    forall l,
      (exists from val released na,
          (<<GET: Memory.get l (tm l) mem = Some (from, Message.message val released na)>>)) \/
      (<<EQ: l = loc /\ tm l = ts>>)
  .
  ...
  Proof.
    ...
    unfold TimeMap.join, Time.join. des; des_ifs; eauto 10.
  Qed.
  ...
  Lemma semi_closed_timemap_add tm mem0 loc from ts val released na mem1
        (CLOSED: semi_closed_timemap tm mem0 loc ts)
        (ADD: Memory.add mem0 loc from ts (Message.message val released na) mem1)
    :
      Memory.closed_timemap tm mem1.
  Proof.
  ...
  Lemma semi_closed_timemap_future tm mem0 loc ts mem1
        (CLOSED: semi_closed_timemap tm mem0 loc ts)
        (FUTURE: Memory.messages_le mem0 mem1)
    :
      semi_closed_timemap tm mem1 loc ts.
  Proof.
    ii. specialize (CLOSED l). des.
    - eapply FUTURE in GET. left. eauto.
    - subst. eauto.
  Qed.
  ...
  Lemma semi_closed_view_add vw mem0 loc from ts val released na mem1
  ...
  Lemma semi_closed_opt_view_add vw mem0 loc from ts val released na mem1
  ...
  | semi_closed_message_concrete
      val released na mem loc ts
      (CLOSED: semi_closed_opt_view released mem loc ts):
      semi_closed_message (Message.message val released na) mem loc ts
  ...
  Lemma semi_closed_message_add vw mem0 loc from ts val released na mem1
  ...
  Proof.
    ...
    unfold Time.join in Heqt. des_ifs; des; clarify; eauto 8.
...
Lemma other_promise_unchangable gl ths tid1 tid2 st1 st2 tvw1 prom1 tvw2 prom2 rsv1 rsv2
      (CWF: Configuration.wf (Configuration.mk ths gl))
      (TID1: IdentMap.find tid1 ths = Some (st1, Local.mk tvw1 prom1 rsv1))
      (TID2: IdentMap.find tid2 ths = Some (st2, Local.mk tvw2 prom2 rsv2))
      (DIFF: tid1 <> tid2)
  :
    Messages.of_memory rsv2 <4= unchangable gl.(Global.memory) rsv1.
Proof.
  ii. inv PR.
  inv CWF. inv WF. destruct st1, st2. econs; eauto.
  - exploit THREADS; try apply TID2; eauto. intros LCWF. inv LCWF. eauto.
  - destruct (Memory.get x0 x1 rsv1) eqn:GET0; eauto. exfalso.
    exploit DISJOINT; eauto. intros LCDISJ. inv LCDISJ. destruct p.
    inv RESERVES_DISJOINT. exploit DISJOINT0; eauto. intros x5. des.
...
Section BOOLMAP.
  Lemma bool_map_le_exists loc (bm0 bm1 bm2: BoolMap.t)
        (ADD: BoolMap.add bm1 loc bm2)
        (LE: BoolMap.le bm0 bm1)
    :
    exists bm, BoolMap.add bm0 loc bm.
  Proof.
    eapply BoolMap.add_exists.
    destruct (bm0 loc) eqn:EQ; auto.
    eapply LE in EQ. inv ADD. clarify.
  Qed.

  Lemma bool_map_le_implb (bm0 bm1: BoolMap.t)
        (LE: BoolMap.le bm0 bm1)
    :
    forall loc, implb (bm0 loc) (bm1 loc).
  Proof.
    i. specialize (LE loc).
    destruct (bm0 loc) eqn:EQ0, (bm1 loc) eqn:EQ1; ss.
    eapply LE. auto.
  Qed.
End BOOLMAP.


Section RTCCONSISTENT.
  Definition rtc_consistent lang (th0: Thread.t lang): Prop :=
    exists th1, (<<STEPS: rtc (@Thread.all_step _) th0 th1>>) /\ (<<CONSISTENT: Thread.consistent th1>>).

  Lemma consistent_rtc_consistent lang (th: Thread.t lang)
        (CONS: Thread.consistent th)
    :
    rtc_consistent th.
  Proof.
    unfold rtc_consistent. esplits.
    { refl. }
    { eauto. }
  Qed.

  Lemma bot_rtc_consistent lang (th: Thread.t lang)
        (CONS: th.(Thread.local).(Local.promises) = BoolMap.bot)
    :
    rtc_consistent th.
  Proof.
    unfold rtc_consistent. esplits.
  ...
  Lemma racy_cap_of lc gl loc to ord
        (RACE: Local.is_racy lc gl loc to ord)
    :
    Local.is_racy lc (Global.cap_of gl) loc to ord.
  Proof.
    inv RACE.
    { econs 1; eauto. }
    { econs 2; eauto. eapply Global.cap_le; eauto. }
  Qed.

  Lemma event_step_rtc_consistent lang (th0 th1: Thread.t lang) e
        (STEP: Thread.step e th0 th1)
        (EVENT: ThreadEvent.get_machine_event e <> MachineEvent.silent)
    :
    rtc_consistent th0.
  Proof.
    destruct (ThreadEvent.get_machine_event e) eqn:EQ; ss.
    { eapply bot_rtc_consistent. inv STEP; inv LOCAL; ss. inv LOCAL0. auto. }
    unfold rtc_consistent. esplits.
    { refl. }
    econs 1.
    { econs; eauto.
      instantiate (1:=@Thread.mk _ th1.(Thread.state) th1.(Thread.local) (Global.cap_of th0.(Thread.global))).
      inv STEP; [inv LOCAL|]; ss. econs 2; ss.
      inv LOCAL; ss.
      { econs; eauto. }
      { econs. inv LOCAL0. econs. eapply racy_cap_of; eauto. }
      { econs. inv LOCAL0.
        { econs 1; eauto. }
        { econs 2; eauto. }
        { econs 3. eapply racy_cap_of; eauto. }
      }
    }
  Qed.

  Lemma step_rtc_consistent lang th0 th1 e
        (STEP: @Thread.step lang e th0 th1)
        (CONS: ThreadEvent.get_machine_event e = MachineEvent.silent -> rtc_consistent th1)
    :
    rtc_consistent th0.
  Proof.
    destruct (ThreadEvent.get_machine_event e) eqn:EVT.
    { hexploit CONS; eauto. i. unfold rtc_consistent in *. des.
      esplits.
      { econs 2; eauto. econs; eauto. }
      { eauto. }
    }
    { eapply event_step_rtc_consistent; eauto. rewrite EVT. ss. }
    { eapply event_step_rtc_consistent; eauto. rewrite EVT. ss. }
  Qed.

  Lemma opt_step_rtc_consistent lang th0 th1 e
        (STEP: @Thread.opt_step lang e th0 th1)
        (CONS: ThreadEvent.get_machine_event e = MachineEvent.silent -> rtc_consistent th1)
    :
    rtc_consistent th0.
  Proof.
    inv STEP; auto. eapply step_rtc_consistent; eauto.
  Qed.

  Lemma internal_step_rtc_consistent lang th0 th1
        (STEP: @Thread.internal_step lang th0 th1)
        (CONS: rtc_consistent th1)
    :
    rtc_consistent th0.
  Proof.
    inv STEP. eapply step_rtc_consistent; eauto.
  Qed.

  Lemma program_step_rtc_consistent lang th0 th1 e
        (STEP: @Thread.program_step lang e th0 th1)
        (CONS: rtc_consistent th1)
    :
    rtc_consistent th0.
  Proof.
  ...
  Lemma internal_steps_rtc_consistent lang th0 th1
        (STEPS: rtc (@Thread.internal_step lang) th0 th1)
        (CONS: rtc_consistent th1)
    :
    rtc_consistent th0.
  Proof.
    revert CONS. induction STEPS; ss.
    i. eapply IHSTEPS in CONS. eapply internal_step_rtc_consistent; eauto.
  Qed.

  Lemma tau_steps_rtc_consistent lang th0 th1
  ...
  Proof.
    revert CONS. induction STEPS; ss.
    i. eapply IHSTEPS in CONS. inv H. eapply step_rtc_consistent; eauto.
  Qed.

  Lemma all_steps_rtc_consistent lang th0 th1
        (STEPS: rtc (@Thread.all_step lang) th0 th1)
        (CONS: rtc_consistent th1)
    :
    rtc_consistent th0.
  ...
  Lemma failure_rtc_consistent lang (th: Thread.t lang)
        (FAILURE: Thread.steps_failure th)
    :
    rtc_consistent th.
  Proof.
    inv FAILURE. eapply tau_steps_rtc_consistent; eauto.
    eapply event_step_rtc_consistent; eauto. rewrite EVENT_FAILURE. ss.
  Qed.
End RTCCONSISTENT.

Lemma loc_fun_add_spec A (f: @LocFun.t A) loc0 a loc1
  :
  LocFun.add loc0 a f loc1 = if Loc.eq_dec loc1 loc0 then a else f loc1.
Proof.
  apply LocFun.add_spec.
Qed.

Lemma rtc_join A (R: A -> A -> Prop) a0 a1
      (RTC: rtc (rtc R) a0 a1)
  :
  rtc R a0 a1.
Proof.
  induction RTC; auto. etrans; eauto.
Qed.
...
Lemma other_promise_included gl ths tid1 tid2 st1 st2 tvw1 prom1 tvw2 prom2 rsv1 rsv2
      (WF: Configuration.wf (Configuration.mk ths gl))
      (FIND1: IdentMap.find tid1 ths = Some (st1, Local.mk tvw1 prom1 rsv1))
      (FIND2: IdentMap.find tid2 ths = Some (st2, Local.mk tvw2 prom2 rsv2))
      (NEQ: tid1 <> tid2)
  :
  BoolMap.le (prom2) (BoolMap.minus gl.(Global.promises) prom1).
Proof.
  inv WF. inv WF0. ss. destruct st1, st2.
  exploit DISJOINT; eauto. intros DISJ.
  ii. unfold BoolMap.minus, andb, negb. des_ifs.
  { inv DISJ. exfalso. eapply PROMISES_DISJOINT; eauto. }
  { eapply THREADS in FIND2. eapply FIND2 in LHS. clarify. }
Qed.
